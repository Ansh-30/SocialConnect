...
import { Heart, Share2, Bookmark, BookmarkCheck, ArrowLeft, Edit3, Trash2, Loader2, X, Image } from 'lucide-react';
...
export default function PostDetail() {
  ...
  const fetchPost = async () => {
    ...
    try {
      ...
      const p = r.data.post;
      setPost(p);
      setLiked(p.likes?.map(String).includes(me?._id));
      setLikes(p.likes?.length || 0);
      setSaved(me?.savedPosts?.map(String).includes(p._id));
  ...
  const toggleSave = async () => {
    ...
    try { await api.post(`/users/save/${id}`); toast.success(prev ? 'Unsaved' : 'Saved!'); }
  ...
  const saveEdit = async () => {
    setSaving(true);
    try {
      const r = await api.put(`/posts/${id}`, { caption, tags });
      setPost(r.data.post);
      setEditing(false);
  ...
  const deletePost = async () => {
    ...
    try { await api.delete(`/posts/${id}`); toast.success('Deleted'); navigate('/'); }
    catch { toast.error('Failed'); }
  };
  ...
  return (
    <div className="space-y-4 animate-fade-up">
      {/* Back */}
      <button onClick={() => navigate(-1)} className="btn-ghost pl-0 text-[--text-3]">
      ...
      <div className="card p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <Link to={`/profile/${post.user?.username}`} className="flex items-center gap-3 group">
            ...
            <div>
              <p className="font-semibold text-[--text-1] group-hover:text-ink-400 transition-colors">{post.user?.name}</p>
          ...
          {isOwner && (
            <div className="flex gap-2">
              ...
              <button onClick={deletePost} className="btn-ghost py-1.5 px-3 text-xs text-red-500 hover:bg-red-500/10">
        ...
        {editing ? (
          <div className="space-y-3 mb-4">
            <textarea value={caption} onChange={e => setCaption(e.target.value)}
              rows={4} maxLength={2200} className="field resize-none" placeholder="Caption…" />
            <input value={tags} onChange={e => setTags(e.target.value)}
              placeholder="Tags: react, design" className="field" />
            <div className="flex gap-2">
              <button onClick={saveEdit} disabled={saving} className="btn-primary py-2 px-4">
          ...
          <>
            {post.caption && (
              <p className="text-[--text-1] leading-relaxed whitespace-pre-line mb-4">{post.caption}</p>
            )}
            {post.tags?.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-4">
                {post.tags.map(t => (
                  <Link key={t} to={`/search?q=%23${t}`} className="tag">#{t}</Link>
                ))}
              </div>
            )}
          </>
...
{post.image && (
  <img
    ...
    alt="Post"
    className="w-full rounded-xl object-cover max-h-[600px] mb-4"
    onError={(e) => {
      e.target.style.display = "none";
      console.log("Post image failed:", post.image);
    }}
  />
)}
        {/* Actions */}
        <div className="flex items-center gap-1 pt-4 border-t border-[--border]">
          <button onClick={toggleLike} className={`post-btn ${liked ? '!text-red-500' : ''}`}>
            <Heart className={`w-5 h-5 ${liked ? 'fill-current' : ''}`} />
            {fmtCount(likes)} Likes
          </button>
          <button onClick={share} className="post-btn">
            <Share2 className="w-5 h-5" />
            {fmtCount(post.shares || 0)} Shares
          </button>
          <button onClick={toggleSave} className={`post-btn ml-auto ${saved ? '!text-ink-400' : ''}`}>
